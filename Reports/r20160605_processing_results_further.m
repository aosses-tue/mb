...
%
% 1. Description:
%       Processing of my experimental data, as published in ISMRA paper.
%       Comments on triadic comparisons:
%           - Only S09 was excluded, since not all her data had been taken before starting the paper
%       Comments on instrument-in-noise:
%           - S07 only experiments A and B were accounted for (C, D were backed up later)
%           - S09, experiments A and B were added (this might not have been the best idea)
%           - S10 was excluded, because many datapoints were above 20 dB SNR, only few thresholds were below (but still above 15 dB)
%           - S15, only experiment A accounted for, 'piano_multi-S15-B-adaptive-S15.apr did not have all staircases finished
%               I do not remember why did I exclude experiments C and D.
%           - There was an issue with the median (threshold) value for the piano pairs 12 and 25,
%               they should have been sorted differently (with respect to what I sent to AK as a first draft: 20160621-ISMRA_2016_draft-1.pdf)
%       
% 2. Stand-alone example:
%       r20160605_processing_results_further;
% 
% 3. Additional info:
...
h = [];
hname = [];
dirmain = 'D:\Documenten-TUe\02-Experiments\2016-APEX-my-experiments\Piano\Main-ICRA-v3\';
if bDoTriadic
...
if bDoStaircase
    ...
    [Rho  pvalue ] = corr(X,Y,'type','Pearson'); % Equivalent to: Rho = Correlation(X,Y,'coefficient');
    [RhoS pvalueS] = corr(X,Y,'type','Spearman');
