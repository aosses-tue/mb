...
%       nsigs = 1; % (default)
%       noisetype = 'white'; % default
%       outsig = noise(samples,nsigs,noisetype);
% 
%       t = (1:length(outsig))/fs;
%       plot(t,outsig)
%       title(sprintf('%s-noise, rms = %.1f dBFS',noisetype,rmsdb(outsig)))
%
%       % Run following lines if you want to store the audio file:
%       try
%           filename = [Get_TUe_paths('outputs') noisetype];
%       catch
%           filename = noisetype;
%       end
...
try
    ...
    warning('narginchk not found, probably you are using old version of MATLAB')
end
