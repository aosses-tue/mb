...
%
% Original file name: rmsilence.m
...
for sno=1:1
    ...
    % if (fs ~=48000)
         sig_r=sig(:,sno); %resample(sig(:,sno),ufs,fs);
    % end
    ...
    if (nargin ==3)
        thresh=threshin*(prctile(env,99)); % Original: prctile(env,50)
    else
...
act_indexo=indx; %resample((indx-1)*fs/ufs, fs,ufs,0);% resampling it to Original rate to reduce the distortions in the extracted signal
