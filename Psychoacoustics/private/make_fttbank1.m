...
%       It is an efficient way to implement an FTT filterbank or Gammatone 
%       filterbank.
%       Note that this implementation results in filters with less stop 
%       band attenuation than complex modulation filterbanks.
%
%       Input parameters:
%       - fs:   sampling frequency fs [Hz]
%       - cf:   column vector with 24 center-frequencies (optional, default: 
%               Bark center-frequencies according to Zwicker)
%       - B :   column vector with 24 critical band widths (optional, 
%               default: Bark bandwidths according to Zwicker). Note that in
%               the PsySound implementation B was removed as input parameter.
%               Fixed values equal to 0.887*ERB are used instead.
%
% References:
...
gain = abs(  ( -2*exp(4*i*cf*pi*T)*T + 2*exp(-(B*T) + 2*i*cf*pi*T).*T.* ...
       ...
       % CONTINUAR AQUI:
% gain_num = ( -2*exp(4*i*cf*pi*T)*T + 2*exp(-(B*T) + 2*i*cf*pi*T).*T.* (cos(2*cf*pi*T) - sqrt(3 - 2^(3/2))*sin(2*cf*pi*T)) ) .* ...
%            ( -2*exp(4*i*cf*pi*T)*T + 2*exp(-(B*T) + 2*i*cf*pi*T).*T.* (cos(2*cf*pi*T) + sqrt(3 - 2^(3/2))*sin(2*cf*pi*T)) ) .* ...
%            ( -2*exp(4*i*cf*pi*T)*T + 2*exp(-(B*T) + 2*i*cf*pi*T).*T.* (cos(2*cf*pi*T) - sqrt(3 + 2^(3/2))*sin(2*cf*pi*T)) ) .* ...
%            ( -2*exp(4*i*cf*pi*T)*T + 2*exp(-(B*T) + 2*i*cf*pi*T).*T.* (cos(2*cf*pi*T) + sqrt(3 + 2^(3/2))*sin(2*cf*pi*T)) );
% gain_den = ( -2./exp(2*B*T) - 2*exp(4*i*cf*pi*T) + 2*(1 + exp(4*i*cf*pi*T)).*exp(B*T)).^4;
%         
% gainL = abs(  ( -2*exp(4*i*cf*pi*T)*T + 2*exp(-(B*T) + 2*i*cf*pi*T).*T.* (cos(2*cf*pi*T) - sqrt(3 - 2^(3/2))*sin(2*cf*pi*T)) ) .* ...
%               ( -2*exp(4*i*cf*pi*T)*T + 2*exp(-(B*T) + 2*i*cf*pi*T).*T.* (cos(2*cf*pi*T) + sqrt(3 - 2^(3/2))*sin(2*cf*pi*T)) ) .* ...
%               ( -2*exp(4*i*cf*pi*T)*T + 2*exp(-(B*T) + 2*i*cf*pi*T).*T.* (cos(2*cf*pi*T) - sqrt(3 + 2^(3/2))*sin(2*cf*pi*T)) ) .* ...
%               ( -2*exp(4*i*cf*pi*T)*T + 2*exp(-(B*T) + 2*i*cf*pi*T).*T.* (cos(2*cf*pi*T) + sqrt(3 + 2^(3/2))*sin(2*cf*pi*T)) ) ./ ...
