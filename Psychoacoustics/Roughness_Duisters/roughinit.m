...
%
function [] = roughinit(fs, switches, twin, dN);
...
if nargin < 4
    if switches(4) == 3
        dN = [round(0.3*fs) round(0.3*fs+fs/70)]; % 1 cycle
    else
...
if switches(4) == 3
    ...
    save roughinit fs numH7 denH7 numH14 denH14 numH30 denH30 ...
        numH36 denH36 numH66 denH66 switches R01
else
    ref1 = (1 + sin(2 * pi * 70 * t)) .* sin(2 * pi * 1e3 * t);
    win = sin(0.5 * pi * 50 .* (0:1/fs:20e-3)).^2;
    ref1 = [ref1(1:length(win)) .* win ref1(length(win)+1:length(ref1)-length(win)) ...
