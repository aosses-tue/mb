...
%
% usage
%   [BM MF Lp] = PemoPreProcInit(BM, MF, fs, fsIR)
%
% input
...
function [BM MF Lp] = PemoPreProcInit(BM, MF, fs, fsIR);
...
switch MF.style
    case 'mfbtd'
        [MF.CenterFreq, out] = mfbtd(1,min(MF.low),max(MF.up),MF.lpco,fsIR);
    case 'lp'
    ...
    otherwise
        error('PemoPreProcInit: illegal MF.style')
end
