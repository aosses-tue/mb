function experiment_report_20131023_pooled
% function experiment_report_20131023_pooled
%
% Pooled data session
%
...
info = getpaths;
addpath( [info.svn.MATLAB 'Classes/'] ); % Matthias GUI
addpath(  info.TB_SP15); % Ensure_field
addpath(  info.TB_APEX);
addpath(  info.TB_APEX_tools);

hoofd_folder    = info.result_folder;
...
if bDoSpeechTests == 1
    ...
    stPlot.xTickLabel   = {'RP','MB','PM','JL','DW','JBD'};
    stPlot.xTickLabelPaper = {'S11','S12','S13','S14','S15','S16'};
    ...
    % stPlot.xTickLabel   = {'RP','MB','PM','JL','DW','JBD'};
    stPlot.xTickLabel   = {'S11','S12','S13','S14','S15','S16'};
    stPlot.xTick        = 1:length(stPlot.xTickLabel);
    stPlot.xLim         = [ 0   max(stPlot.xTick)+2];
    stPlot.XLabel       = 'Subject';
    ...
    stPlot.figPos       = [0 0 1024 450];
    stPlot.xTickLabel   = {'MB','PM','JL','DW','JBD'};
    stPlot.xTickLabelPaper = {'S12','S13','S14','S15','S16'};
    stPlot.xTick        = 1:length(stPlot.xTickLabel);
    stPlot.xLim         = [ 0   max(stPlot.xTick)+2];
    stPlot.XLabel       = 'Subject';
    ...
    for k = 1:2
        ...
        if SNR(k) == 10
            stPlot.TitleHead    = ['Flemish Matrix, SNR = 10 dB (Results for ' SubjectName ')'];
        else
            stPlot.TitleHead    = ['Flemish Matrix, SNR = 5 dB (Results for ' SubjectName ')']; 
        end
