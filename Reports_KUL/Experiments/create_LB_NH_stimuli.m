function [wavfilename calfilename DirAudioFiles] = create_LB_NH_stimuli(idle, tones, instr, bGenerateTones, bA_weighting)
% function [wavfilename calfilename DirAudioFiles] = create_LB_NH_stimuli(idle, tones)
%
...
for k=1:length(instr)
    ...
    for i = 5 % i = 5 corresponds to calibration tone
        ...
        if bA_weighting == 0
            ...
            calfilenames    = {[ instr{k} '-A_' num2str(freq_cal) '_Hz-cal']};
        end
    ...
    for i = 1:length( tones )
        ...
        if bA_weighting == 0
            ...
            wavfilenames{1,i} = [ instr{k},'-A_', tones{2, i} '_Hz'];
        end
