function ac = PlotMeans(stPlot, x1, x2, x1b, x2b)
% function PlotMeans(stPlot, x1, x2, x1b, x2b)
%
...
% Title               = ['Pitch Ranking, Ref. '];     stPlot.Title = Title;
% stPlot.YLabel       = '% Correct';
% stPlot.XLabel       = 'Comparison tones [Hz]';
% stPlot.ReverseData  = 1; % from worst to best score
% stPlot.xTick        = xTick;
% stPlot.yTick        = yTick;
% stPlot.Title        = {'PR ref 131 [Hz]',''PR ref 208 [Hz]''};
%
...
try
    ...
    nReferences  = 5; % Gives the number of plots
end
...
stPlot = Ensure_field(stPlot,'xGap'          , 10*stPlot.scalerate);
stPlot = Ensure_field(stPlot,'yGap'          , 60*stPlot.scalerate);
stPlot = Ensure_field(stPlot,'margins'       , [70 70 50 70]*stPlot.scalerate);
stPlot = Ensure_field(stPlot,'figPos'        , [0 0 1024 768/2]*stPlot.scalerate);
stPlot = Ensure_field(stPlot,'SeriesColor'   , {'k'  , 'w'});
stPlot = Ensure_field(stPlot,'SeriesLabel'   , {'Serie 1','Serie 2'});
...
stPlot = Ensure_field(stPlot,'ReverseData'   , 0);
stPlot = Ensure_field(stPlot,'separation_series', 0.1*stPlot.scalerate);
stPlot = Ensure_field(stPlot,'XGrid'         ,'off');
stPlot = Ensure_field(stPlot,'YGrid'         ,'off');
stPlot = Ensure_field(stPlot,'line_type'     ,':'); % line type used to connect data points of the same serie
stPlot = Ensure_field(stPlot,'line_color'    ,repmat(0.75, 1, 3));
stPlot = Ensure_field(stPlot,'bPlotIndividual',0); 
stPlot = Ensure_field(stPlot,'bAddPanelLabel',1);
...
ColorLines  = 'k'; 
stPlot  = Ensure_field(stPlot,'fntsz',12*stPlot.scalerate);
...
if length(stPlot.Title) == 1
    try
        Title = name2figname( stPlot.Title );
    catch
        Title = stPlot.Title;
    end
else
    for i = 1:length(stPlot.Title)
        Title{i} = name2figname( stPlot.Title{i} );
    end
end
...
stPlot      = Ensure_field(stPlot, 'markerSize', round(0.005*figPos(3))); 
...
for i = CountFig
    ...
    if stPlot.ReverseData == 1
        ...
        try
            DataSerie1  = x1(:, (1:length(xTick)) + length(xTick)*(i-1) );
            DataSerie2  = x2(:, (1:length(xTick)) + length(xTick)*(i-1) );
            disp([mfilename '.m: for Physical validation, x1b and x2b were replaced by x1 and x2... maybe you will get wrong plots'])
            pause(1)
        end
    end
    ...
    if nargin >= 5
        if stPlot.ReverseData == 1
            DataSerie1b  = x1b(:, (length(xTick):-1:1) + length(xTick)*(i-1) ); % Reverse the data (before: from interval 4 to 1, after: from 1 to 4)
            DataSerie2b  = x2b(:, (length(xTick):-1:1) + length(xTick)*(i-1) );
        else
            DataSerie1b  = x1b(:, (1:length(xTick) + length(xTick)*(i-1)));
            DataSerie2b  = x2b(:, (1:length(xTick) + length(xTick)*(i-1)));
        end
    end
    ...
    pc.axesConfig(row,col).ax_fontSize = stPlot.fntsz;
    ...
    pcplt1 = PercentCorrectPlotter(     ac, ...                            % ac is the figure object
                                        ...
                                        0, ...
                                        {}); 
    pcplt1.setupMarkers('s', stPlot.markerSize, SeriesColor{1}, SeriesLabel{1}); % 1 = ACE
    pcplt1.setupLines(stPlot.line_type, 1, stPlot.line_color, '');
    pcplt1.setupErrorbars('-', 1, ColorLines, 0.1*stPlot.scalerate);
    pcplt1.plot();
    
    if nargin >= 5
        pcplt1 = PercentCorrectPlotter( ac, ...                            % ac is the figure object
                                        DataSerie1b, ... % aceData (size = N x M), N subjects, M different trials
                                        separation_series, ...
                                        0, ...
                                        {}); 
        pcplt1.setupMarkers('s', stPlot.markerSize, SeriesColor{1}, SeriesLabel{1}); % 1 = ACE
    ...
    pcplt2 = PercentCorrectPlotter(     ac, ...
                                        ...
                                        0, ...
                                        {});
    pcplt2.setupMarkers('s', stPlot.markerSize, SeriesColor{2}, SeriesLabel{2});
    pcplt2.setupLines(stPlot.line_type, 1, stPlot.line_color, '');
    pcplt2.setupErrorbars('-', 1, ColorLines, 0.1*stPlot.scalerate);
    pcplt2.plot()
    
    if stPlot.bPlotIndividual == 1
        hold on;
        xTemp = ( 1:size(DataSerie1,2) )-separation_series;
        plot(xTemp, DataSerie1, 'rx', 'LineWidth',0.2*stPlot.scalerate)
        try
            xTemp = ( 1:size(DataSerie2,2) )+separation_series;
            plot(xTemp, DataSerie2, 'rx', 'LineWidth',0.2*stPlot.scalerate)
        catch
            warning('It was not possible to plot one of the data series')
        end
    ...
    if stPlot.sameYAxis == 0 || i==1
        ...
        if ~strcmp(SeriesLabel{1},'') && ~strcmp(SeriesLabel{2},'')
            hlabel = legend([h1(1) h2(1)], SeriesLabel, 'location', LocationLegend);
            set(hlabel,'FontSize',stPlot.fntsz);
        end
        
        try
            yl1 = YLabelConfig(pc.axesConfig(1,i), YLabel{i}, yGap+60*stPlot.scalerate, 'left', 'middle', 90, stPlot.fntsz); 
        catch % Then YLabel is not a cell array
            yl1 = YLabelConfig(pc.axesConfig(1,1), YLabel, yGap, 'left', 'middle', 90, stPlot.fntsz); 
        end
        
    elseif mod(i,2) == 1
        
        yl1 = YLabelConfig(pc.axesConfig(row,1), YLabel, yGap, 'left', 'middle', 90, stPlot.fntsz);
    ...
    if nargin >= 5
        xPositions = xLim;  
        yPositions = [0 0];
        l1 = LineCurve(ac, xPositions, yPositions, '-', 'k', 2*stPlot.scalerate, ''); % gris = [0.75 0.75 0.75]
        l1.plot();
    end
    
    try
        label = [label ' ' stPlot.Title{i}];
    catch
        label = [label ' ' stPlot.Title];
    end
    ...
    if stPlot.bTruncateName == 1
        if length(label)>40
            ...
            display('Filename truncated while plotting')
                   ...
                   % (axesConfig,             string, xGap, reAxisHorizontal, reAxisVertical, rotation, fontSize    )
    xl = XLabelConfig(pc.axesConfig(row,col),  label, xGap, positionLabel   ,          'top',        0, stPlot.fntsz); 
    xl.plot();
    
    if i <= 2 && length(CountFig) <= 2
        xl = XLabelConfig(pc.axesConfig(row,col), XLabel, xGap, 'middle', 'bottom', 0, stPlot.fntsz);  
        xl.plot()
    end
    ...
    if stPlot.bAddPanelLabel
        tl = TextLabel(ac, ID, [0.1 0.1], 'normalized', 0, '', stPlot.fntsz);
        tl.plot();
    end
