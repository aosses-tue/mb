...
if length(obj.I_Handles) > 0
    s.Handles = obj.I_Handles;
else
...
for k = 1:length(s.Handles)
    if strcmp(get(s.Handles(k),'Type'),'axes')
        av = [av s.Handles(k)];
        disp([mfilename '.m: check this script, still in process to be adapted'])
        pause()
    elseif strcmp(get(s.Handles(k),'Type'),'figure');
        ...
        for j = numel(fc):-1:1
            ...
            if numel(findobj(fc(j),'type','axes','-and','-not','parent',obj.hOutFig,'-and','-not','Tag','legend','-and','-not','Tag','Colorbar'))>0
                av = [av fc(j)]; %axes to plot
            elseif numel(findobj(fc(j),'type','axes','-and','-not','parent',obj.hOutFig,'-and','Tag','legend','-and','-not','Tag','Colorbar'))>0
...
if isprop(obj,'I_Handles')
    ...
    hAxes = findobj('type','axes','-and','-not','parent',obj.hOutFig,'-and','parent',obj.I_Handles);
    hLeg = findobj(hAxes,'Tag','legend','-and','-not','parent',obj.hOutFig,'-and','parent',obj.I_Handles); % only legend axes
    hColorbar = findobj(s.Handles,'type','axes','-and','Tag','Colorbar','-and','-not','parent',obj.hOutFig,'-and','parent',obj.I_Handles);
    ...
    hColorbar = findobj(s.Handles,'type','axes','-and','Tag','Colorbar','-and','-not','parent',obj.hOutFig);
    hColorbar = sort(hColorbar);
    cud = get(hLeg,'UserData');
...
%%
for i = 1:numel(hAxes)
    set(hAxes(i),'XTickLabelMode','Auto')
end
...
set(obj.hOutFig,'Colormap',get(s.Handles(1),'Colormap'))
%%
...
if numel(hColorbar)>0
    for k = 1:naxes
        set(0,'CurrentFigure',obj.hOutFig)
        caxeidx = orderaxes(k);
        axes(na(k));
        cbar = colorbar;
        barchild = get(hColorbar(k),'children');
        cbarYTick = get(hColorbar(k),'YTick');
        %ydatabar = get(barchild,'YData');
        cdatabar = get(barchild,'CData');
        %maxindex = floor(max(ydatabar)*max(cdatabar)) - 1;
        %minindex = floor(min(ydatabar)*max(cdatabar)) - 1;
        set(get(cbar,'children'),'CData',cdatabar);
        set(cbar,'YTick',cbarYTick);
    end
end
