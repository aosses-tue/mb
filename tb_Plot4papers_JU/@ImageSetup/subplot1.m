function Hgcf = subplot1(obj,M,N,varargin);
%-------------------------------------------------------------------------
% Hgcf = subplot1(obj,M,N,varargin) - An improved subplot function
%
% Inputs  : - If more than one input arguments are given:
%               M - Number of rows
%               N - Number of columns
%
%           - If single input argument is given, then the first input is the
%            subplot-number for which to set focus. This could a scalar or two element vector (I,J).
%
...
switch MoveFoc
 ...
 case 0
    ...
    Xmin   = Min(1); % between 0 and 1 (tending to 0)
    Ymin   = Min(2); % between 0 and 1 (tending to 0)
    Xmax   = Max(1); % between 0 and 1 (tending to 1)
    Ymax   = Max(2); % between 0 and 1 (tending to 1)
    Xgap   = Gap(1);
    Ygap   = Gap(2);
    ...
    Ptot    = M.*N;             % Total number of plots
    Hgcf    = figure(obj.hOutFig);
    ...
    for Pi  = 1:1:Ptot,
        ...
        Row = ceil(Pi./N);       % Gives the row    of the subplot
        Col = Pi - (Row - 1)*N;  % Gives the column of the subplot 
...
%       hold on;
        ...
        PosRect = [Xstart,Ystart,Xbox,Ybox];
        axes('position', PosRect);
