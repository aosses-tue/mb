function [inf, out] = mfbtd_drnl(in,lmf,umf,style,fs)
% function [inf, out] = mfbtd_drnl(in,lmf,umf,style,fs)
% 
% 1. Description:
%       mfbtd_drnl.m - applies modulation filterbank as suggested in Dau et 
%       al. 1997 with additional 150 Hz LP.
% 
%       Usage: [inf, out] = mfbtd(in,lmf,umf,style,fs)
%       Where:
%           - in    = input column vector.
%           - lmf   = lowest modulation-filter center frequency, if 0 the 
%                     output of a 2nd-order Butterworth lowpass filter is 
%                     additionally computed.
%           - umf   = highest modulation-filter center frequency, for typical 
%                     applications choose umf = 1500. 
%                  ** If lmf = umf only the output of a single filter at lmf is computed.
%           - style = determines fc of the lowpass filter: 1 = 2.5 Hz, 2 = 7.5 Hz.
%           - fs    = sampling rate in Hz, should be greater than 8000 Hz 
%                     to avoid aliasing errors.
%
%           - inf	= center frequencies of the modulation filters.
%           - [out1,out2, ...,outn] = each column of matrix out contains 
%                     the output of a single modulation filter.
%
