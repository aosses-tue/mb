function pwr = Band_power(varargin)
% BAND_POWER Band power
%   P = BAND_POWER(X) computes the average power in the input signal vector,
%   X.  
%
%   P = BAND_POWER(X, Fs, FREQRANGE) specifies FREQRANGE as a two-element
...
%
%   P = BAND_POWER(Pxx, F, 'psd') computes the average power via a rectangle
...
%   vector F.
%
%   P = BAND_POWER(Pxx, F, FREQRANGE, 'psd') specifies FREQRANGE as a
...
try
    narginchk(1,4); % not MATLAB R2009a compatible
end
...
validateattributes(Pxx, {'numeric'},{'vector','finite','real'}, ...
  'bandpower', 'Pxx', 1);
