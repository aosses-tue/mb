function handles = barweb(barvalues, errors, width, groupnames, bw_title, bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend, font_size, ylimits)
% function handles = barweb(barvalues, errors, width, groupnames, bw_title, bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend, font_size, ylimits)
%
% Ex: handles = barweb(my_barvalues, my_errors, [], [], [], [], [], bone, [], bw_legend)
%
% barweb is the m-by-n matrix of barvalues to be plotted.
% barweb calls the MATLAB bar function and plots m groups of n bars using the width and bw_colormap carameters.
...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
	...
	if ~isempty(bw_title)
		handles.title = title(bw_title, 'fontsize', font_size, 'fontweight', 'bold');
	else
		handles.title = [];
	end
	if ~isempty(bw_xlabel)
		handles.xlabel =xlabel(bw_xlabel, 'fontsize', font_size, 'fontweight', 'bold');
	else
		handles.xlabel = [];
	end
	if ~isempty(bw_ylabel)
		handles.ylabel =ylabel(bw_ylabel, 'fontsize', font_size, 'fontweight', 'bold');
	else
		handles.ylabel = [];
	end

	set(gca, 'xticklabel', groupnames, 'box', 'off', 'ticklength', [0 0], 'fontsize', font_size, 'xtick',1:numgroups);
	...
	if ~isempty(bw_legend)
		handles.legend = legend(bw_legend, 'location', 'NorthEastOutside', 'fontsize', font_size);
		legend boxoff;
	else
		handles.legend = [];
    end
