% This function is an implementation of the EPSM-filterbank
% as presented by Ewert & Dau 2000, without the additional lowpass filter
% with a cut-off at 150 Hz. This implementation consists of a lowpass
% filter with a cutoff at 1 Hz, in parallel with 6 bandpass filters with
% octave spacing. the Center-frequencies of the bandpass filters are lower
% than the original from Ewert & Dau (2000). In each of the filters data is
% integrated within the pass-band of the filter to give the power within
% that filter.
% 
% freqs  :  the frequencies of the data, starting from f = 0
% data   :  the powerspectrum data to be filtered.
% fcs    :  the centerfrequencies od the filters
% powers :  the integrated power at the output of each filter
% Wcf    :  matrix containing the frequency-domain squared transfer
...
function [fcs powers] = modFbank(data,fig,fs)
...
X_power_pos(2:end) = X_power_pos(2:end).* (2)  ; %take positive frequencies only and mulitply by two-squared to get the same total energy
...
for k = 1:size(Wcf,1)
    ...
    powers(k) =  sum(Vout(k,2:end))/ N / DC_power;%integrate envelope power in the passband of the filter. Index goes from 2:end since 
