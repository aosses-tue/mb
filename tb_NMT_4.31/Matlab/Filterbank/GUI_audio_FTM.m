...
% u.audio_sample_rate: The sample rate of the audio signals.
...
if nargin == 0
	...
	if file_name	
		load([path_name, file_name]);	% creates 'u'
		GUI_audio_FTM(u);		
	end
	return;
end
...
if isstruct(u)
	...
	u.t = (0:(len-1))/u.audio_sample_rate;	% time scale for horizontal axis
	...
	u.image_axes = axes;
	u.image = image(u.t, [0 u.audio_sample_rate/2], u.mag{u.selection},'CDataMapping','scaled');
...
function Play_selected(u)
	try
		sound(u.audio(u.selection, :), u.audio_sample_rate);
	catch
