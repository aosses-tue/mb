...
%   $Header: //dsp/Nucleus/_Releases/NMT_4.30/Matlab/Processing/Process_test.m#1 $
%   $Change: 86418 $
...
pd.chunk_size = 6000;
pd.output = 'vertcat';
pd.processes = {};			% No processing
y = Process(pd, 'asa.wav');
Tester(y, a);

pd.gains_dB = 20;
pd = Append_process(pd, 'Gain_proc');
pd = Append_process(pd, 'Abs_proc');
y = Process(pd, 'asa.wav');
Tester(y, 10*abs(a));
